...
class AccountMove(models.Model):
    ...
    x_pl_vat_typ_dokumentu = fields.Selection(string='Typ Dokumentu', selection=X_PL_VAT_OUT_TYPES,
                                              help='Oznaczenia dowodu sprzedaży')

    x_pl_vat_dokument_zakupu = fields.Selection(string='Dokument Zakupu', selection=X_PL_VAT_IN_TYPES,
                                                help='Oznaczenie dowodu zakupu')

    x_pl_vat_mpp = fields.Boolean(string='MPP', default=False,
    ...
    # sprzedaż
    x_pl_vat_sw = fields.Boolean(string='SW', default=False,
                                 help='Dostawa w ramach sprzedaży wysyłkowej z terytorium kraju, '
                                      'o której mowa w art. 23 ustawy')
    x_pl_vat_ee = fields.Boolean(string='EE', default=False,
                                 help='Świadczenie usług telekomunikacyjnych, nadawczych i elektronicznych, '
                                      'o których mowa w art. 28k ustawy')
    x_pl_vat_tp = fields.Boolean(string='TP', default=False,
    ...
    x_pl_vat_tt_wnt = fields.Boolean(string='TT-WNT', default=False,
    ...
    x_pl_vat_tt_d = fields.Boolean(string='TT-D', default=False,
                                   help='Dostawa towarów poza terytorium kraju dokonana przez drugiego w kolejności '
                                        'podatnika VAT w ramach transakcji trójstronnej w procedurze uproszczonej, '
                                        'o której mowa w dziale XII rozdział 8 ustawy.')
    x_pl_vat_mr_t = fields.Boolean(string='MR-T', default=False,
                                   help='Świadczenie usług turystyki opodatkowane na zasadach marży '
                                        'zgodnie z art. 119 ustawy.')
    x_pl_vat_mr_uz = fields.Boolean(string='MR-UZ', default=False,
    ...
    x_pl_vat_i42 = fields.Boolean(string='I42', default=False,
                                  help='Wewnątrzwspólnotowa dostawa towarów następująca po imporcie tych towarów '
                                       'w ramach procedury celnej 42 (import).')
    x_pl_vat_i63 = fields.Boolean(string='I63', default=False,
                                  help='Wewnątrzwspólnotowa dostawa towarów następująca po imporcie tych towarów '
                                       'w ramach procedury celnej 63 (import).')
    x_pl_vat_b_spv = fields.Boolean(string='B-SPV', default=False,
    ...
    x_pl_vat_b_spv_dostawa = fields.Boolean(string='B-SPV Dostawa', default=False,
    ...
    x_pl_vat_b_mpv_prowizja = fields.Boolean(string='B-MPV Prowizja', default=False,
    ...
    x_pl_vat_korekta_podstawy_opodt = fields.Boolean(string='Korekta Podstawy Opodatkowania', default=False,
                                                     help='Korekta podstawy opodatkowania oraz podatku należnego, '
                                                          'o której mowa w art. 89a ust. 1 i 4 ustawy')
    ...
    # zakup
    x_pl_vat_imp = fields.Boolean(string='IMP', default=False,
    ...
    def action_post(self):
        ...
        for move in self:
            if move.is_invoice(include_receipts=True) and not move.pl_vat_date:
                if move.invoice_date and move.move_type in ('out_invoice', 'out_refund') \
...
class AccountMoveLine(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            type_out_invoice = self.env['account.move']\
                                   .browse(vals['move_id']).move_type in ('out_invoice', 'out_refund')
            if type_out_invoice and vals.get('product_id') and not vals.get('exclude_from_invoice_tab'):
