...
class JPKTDocumentType(models.Model):
    ...
    _description = _('JPK Document Type')
    ...
    def constrains_unique_values(self):
        for record in self:
            ...
            if self.search([('system_code', '=', record.system_code),
                            ('schema_version', '=', record.schema_version)], count=True) > 1:
    ...
    def create_attachment(self):
        if not self.xsd_id and self.xsd_id_datas:
            self.xsd_id = self.env['ir.attachment'].create(dict(
                name=self.xsd_id_name,
                type='binary',
                datas=self.xsd_id_datas
            )).id
