...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    ], default='normal', required=1, string='Invoice Type')
    ...
    x_orders_currency_id = fields.Many2one('res.currency', string='Orders Currency',
                                           compute='_x_compute_orders_currency_id')
    ...
    def onchange_advance_payment_method_2(self):
    ...
    def _x_compute_orders_currency_id(self):
        ...
        for wizard in self:
            ...
            wizard.x_is_convertible = (all(currency == currency_ids[0] for currency in currency_ids) and
                                       all(currency_id == pln for currency_id in
                                           wizard.order_ids.mapped('invoice_ids').mapped('currency_id')) and
                                       currency_ids[0] != pln)

            wizard.x_orders_currency_id = currency_ids[0] if wizard.x_is_convertible else False

    @api.onchange('x_convert_to_pln')
    def x_onchange_convert_to_pln(self):
    ...
    def default_get(self, fields):
        output = super(SaleAdvancePaymentInv, self).default_get(fields)
        if not self._x_get_is_poland():
            return output

        orders = self.env['sale.order'].browse(self.env.context['active_ids'])
        output['order_ids'] = [(6, 0, orders.ids)]
        output['invoice_lines'] = [(6, 0, orders.mapped('order_line').filtered(
            lambda l: l.invoice_status == 'to invoice').ids)]
    ...
    def onchange_advance_payment_method(self):
        ...
        if len(self._context.get('active_ids', [])) == 1:
            ...
            for tax in taxes:
                ...
                self.advance_lines = [(0, 0, dict(
                    tax_id=tax.id,
                    original_subtotal=sum(line.price_subtotal for line in lines),
                    original_total=sum(line.price_total for line in lines),
    ...
    def _prepare_invoice_values(self, order, name, amount, so_line):
        ...
        if so_lines:
            invoice_vals['invoice_line_ids'] = [(0, 0, {
                'name': _('Advance payment [%s]') % _line.tax_id[0].description,
                'price_unit': _line.price_unit if not self.x_convert_to_pln else _line.price_unit * self.x_convert_rate.x_rate_inverted,
                'quantity': 1.0,
    ...
    def _create_invoice(self, order, so_line, amount):
        ...
        if self.x_convert_rate:
            invoice_vals['narration'] = _('Rate %s with effective date: %s', self.x_convert_rate.x_rate_inverted,
                                          self.x_convert_rate.name)
    ...
    def _prepare_deposit_product(self):
        output = super(SaleAdvancePaymentInv, self)._prepare_deposit_product()
        if not self._x_get_is_poland():
            return output
    ...
    def _prepare_so_line(self, order, analytic_tag_ids, tax, amount):
        if not self._x_get_is_poland():
            return super(SaleAdvancePaymentInv, self)._prepare_so_line(order, analytic_tag_ids, tax, amount)

        so_values = super(SaleAdvancePaymentInv, self)._prepare_so_line(order, analytic_tag_ids, tax.ids, amount)
    ...
    def create_invoices(self):
        ...
        ctx['selected_invoice_lines'] = self.invoice_lines.ids
        ctx['invoice_type'] = self.advance_payment_method
        ctx['x_journal_id'] = self.x_journal_id.id
        ctx['x_convert_rate'] = self.x_convert_rate.id
        self = self.with_context(ctx)
        sale_orders = self.env['sale.order'].browse(self._context.get('active_ids', []))

        if self.advance_payment_method_2 == 'normal':
            if not self.invoice_lines.filtered(lambda invoice_line: not invoice_line.is_downpayment):
        ...
        if self.advance_payment_method == 'delivered':
            ...
            for order in sale_orders:
                if self.product_id.invoice_policy != 'order':
                    raise UserError(_('The product used to invoice a down payment should have an invoice policy set to '
                ...
                for adv_line in advance_lines:
                    ...
                    so_line_values.append(self._prepare_so_line(order, analytic_tag_ids, adv_line.tax_id, amount))
                    so_lines.append(sale_line_obj.create(so_line_values))
                # noinspection PyProtectedMember
                for so_line, values in zip(so_lines, so_line_values):
                    self.with_context(so_advance_lines=so_lines)._create_invoice(order, so_line, values['price_unit'])
...
class SaleAdvanceLine(models.TransientModel):
    ...
    def onchange_percent(self):
        if 'percent' not in self.env.context:
            return
        value = self.original_subtotal * (self.percent / 100.0)
        value_total = value * (1.0 + (self.tax_id.amount / 100.0))
